...
# 定义数据处理函数
def data_handling1(shared_cumul, shared_y, num_processes, chunk_size, bs, FLAGS, queue, shared_queue):
    while True:
        data = shared_queue.get()
        if data is None:  # 用于退出线程的信号
            break
...
def model_prediction_task(shared_cumul, shared_y, queue, shutdown_flag, FLAGS, series, train_data, chunk_size, num_processes):
    ...
    # 创建并启动数据处理线程
    data_thread = threading.Thread(target=data_handling1, args=(shared_cumul, shared_y, num_processes, chunk_size, bs, FLAGS, queue, shared_queue))
    data_thread.start()
...
def encode(temp_dir, compressed_file, FLAGS, series, train_data, last_train_data):
    ...
    cumul = np.zeros(FLAGS.vocab_size+1, dtype=np.uint64)
    cumul[1:] = np.cumsum(prob*10000000 + 1)
    cumul_b = np.zeros((bs, FLAGS.vocab_size + 1), dtype=np.uint64)
    for i in range(bs):
      cumul_b[i, :] = cumul
    iter_num = len(train_data) // FLAGS.batch_size
    ind = np.array(range(bs)) * iter_num
    ...
    for j in range(FLAGS.seq_len):
        ...
        copy_to_shared_memory(shared_cumul, shared_y, cumul_b, y[j], 0, bs)
